...
def initial_oneSample(parameter_bounds, seed):
    ...
    random.seed(seed)
    df_one = pd.DataFrame({k: random.sample(v, 1)
                          for k, v in dict_listAll.items()})
    return df_one


def initial_batch_8(parameter_bounds, seed):
    # balanced design for the first 8 points, with D-optimality criterion
    saveCSVFileName = os.path.join(
        'temp_initial_batch',
        'df_init_seed_' + str(seed) + '.csv')
    ...
    if os.path.exists(saveCSVFileName):
        ...
        random.seed(seed)
        dict_Reagent_Solvent = {
            'Reagent': parameter_bounds['reagents']['names'],
            'Solvent': parameter_bounds['solvents']['names']}
        df_init_factors = expand_grid(dict_Reagent_Solvent)
        df_init_factors = df_init_factors.sample(frac=1)
        Additive_all = np.random.permutation(
            parameter_bounds['additives']['names'])
        df_init_factors = pd.concat([df_init_factors, df_init_factors], axis=0).reset_index(
            drop=True).assign(Additive=Additive_all)
        det_list = []
        df_init_list = []
        for i in range(1000):
            df_init_i = pd.DataFrame()
            Reagent_equiv_all = np.arange(
                parameter_bounds['reagents']['min_equivalents'],
                parameter_bounds['reagents']['max_equivalents'] +
                parameter_bounds['reagents']['step_size'] /
                10.0,
                step=parameter_bounds['reagents']['step_size'])
            df_init_i = df_init_i.assign(Reagent_equiv=np.random.permutation([Reagent_equiv_all[round(
                x)] for x in np.percentile(range(len(Reagent_equiv_all)), [0, 25, 25, 50, 50, 75, 75, 100])]))
            AdditiveLoading_all = np.arange(
                parameter_bounds['additives']['min_mole_percent'],
                parameter_bounds['additives']['max_mole_percent'] +
                parameter_bounds['additives']['step_size'] /
                10.0,
                step=parameter_bounds['additives']['step_size'])
            df_init_i = df_init_i.assign(AdditiveLoading=np.random.permutation([AdditiveLoading_all[round(
                x)] for x in np.percentile(range(len(AdditiveLoading_all)), [0, 25, 25, 50, 50, 75, 75, 100])]))
            Temperature_all = np.arange(
                parameter_bounds['temperature']['min'],
                parameter_bounds['temperature']['max'] +
                parameter_bounds['temperature']['step_size'] /
                10.0,
                step=parameter_bounds['temperature']['step_size'])
            df_init_i = df_init_i.assign(Temperature=np.random.permutation([Temperature_all[round(
                x)] for x in np.percentile(range(len(Temperature_all)), [0, 25, 25, 50, 50, 75, 75, 100])]))
            df_init_i = df_init_i.assign(
                Stage=random.sample([1, 2, 2, 3, 3, 4, 4, 5], 8))
            df_init_i = df_init_i.assign(
                Reagent_DBDMH=np.array(
                    df_init_factors['Reagent'] == 'DBDMH',
                    dtype='int8'))
            df_init_i = df_init_i.assign(
                Solvent_DMC=np.array(
                    df_init_factors['Solvent'] == 'DMC',
                    dtype='int8'))
        ...
        df_init = pd.concat([df_init_factors,
                             df_init_list[ind_max]],
...
def optFun_LM(
    ...
    for j in np.arange(8, 12):
        max_corr_j = np.max([abs(np.corrcoef(X[:, i], X[:, j])[0, 1])
    ...
    saveModelPath = os.path.join(model_dir, 'LM_' +
                                 initMethod +
                                 '_' +
                                 acquisition +
                                 '_' +
    ...
    if top_index.shape[0] > 1:
        ...
        for i in range(top_index.shape[0]):
            min_dist2old_i = np.min(
                [np.linalg.norm(X_all[top_index[i], :] - X[j, :]) for j in range(X.shape[0])])
        ...
        dist2old_select = np.min(
            [np.linalg.norm(X_all[select_index, :] - X[j, :]) for j in range(X.shape[0])])
    if dist2old_select > 0:
        df_select = df_para.iloc[select_index].assign(
            target=[acquisition],
            target_value=pred_z[select_index],
            target_f=transform_z2f(
                pred_z[select_index])).reset_index(
            drop=True)
...
def optFun_BO(
    ...
    gpr = GaussianProcessRegressor(
        ...
        random_state=123)
    gpr.fit(X, z)
    saveModelPath = os.path.join(model_dir, 'BO_' +
                                 initMethod +
                                 '_' +
                                 acquisition +
                                 '_' +
    ...
    if acquisition == 'EI':
        ...
        acq_values = pred_sd_z * \
            (norm.pdf(gamma_z) - gamma_z * (1 - norm.cdf(gamma_z)))  # EI
    elif acquisition == 'PI':
        ...
        acq_values = pred_z + \
            norm.ppf(1 - (1 - alpha) / 2.0) * pred_sd_z  # upr_z
    top_index = np.argwhere(acq_values == np.amax(
        acq_values))  # np.argmax(acq_values)
    if top_index.shape[0] > 1:
        np.random.shuffle(top_index)
        dist2old = []
        for i in range(top_index.shape[0]):
    ...
    if dist2old_select > 0:
        df_select = df_para.iloc[select_index].assign(
            target=[acquisition],
            target_value=acq_values[select_index]).reset_index(
            drop=True)
        if acquisition == 'EI':
            df_select = df_select.assign(
                target_f=transform_z2f(
                    z_best + acq_values[select_index]))
        elif acquisition == 'PI':
            df_select = df_select.assign(
                target_f=100 * acq_values[select_index])
        else:
            df_select = df_select.assign(
                target_f=transform_z2f(
                    acq_values[select_index]))
        dict_select = df_select.iloc[0].to_dict()
        return dict_select
    else:
...
def optFun_RS(dat, parameter_bounds, seed, n_start=1, n_candidates=100):
    if dat.shape[0] < n_start:
        ...
        #dict_select = df_init.iloc[dat.shape[0]].to_dict()
        dict_select = initial_oneSample(
            parameter_bounds, seed).iloc[0].to_dict()
        return dict_select
    ...
    for i in range(len(candidates)):
        min_dist2old_i = np.min(
            [np.linalg.norm(X_all[i, :] - X[j, :]) for j in range(X.shape[0])])
        dist2old.append(min_dist2old_i)

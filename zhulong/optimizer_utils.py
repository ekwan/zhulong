...
def preprocessing_lm(dat, X_only = True):
    ...
    # add intercept column and quadratic terms for Additive_pKa, Temperature, Reagent_equiv and Stage to X
    ...
    X = np.column_stack((np.ones(shape = (X.shape[0],1)), X, Additive_pKa_quadratic, Temperature_quadratic, Reagent_equiv_quadratic, Stage_quadratic)) 
    cols_X = ['ones']+colnames_Reagent+colnames_Solvent+["AdditiveLoading","Additive_pKa","Temperature","Reagent_equiv","Stage"] + \
                ['Additive_pKa_2', 'Temperature_2', 'Reagent_equiv_2', 'Stage_2']
    if X_only:
        return X, cols_X
    else:
        y = dat[["f"]].to_numpy()
